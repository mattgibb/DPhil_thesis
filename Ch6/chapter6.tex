...
\label{sec:methods}
  ...
  \label{sub:image_preparation}
    ...
    \begin{figure}[htbp]
      ...
      \caption{Block face images of slice 582. The original image is shown in \textbf{(a)}, with \textbf{(b)} Gaussian smoothed and downsampled by a factor of 8.}
    ...
    flipping and rotating
     A small number of slices had been flipped over in the experimental process; these were subject to a reflection along the x-direction. Otherwise normal flip/rotate.
    
     The images provided by Burton et al. yield unprecedented detail and quality. Although every possible step was taken during image acquisition, certain unavoidable experimental practicalities needed correcting for. A handful of images required flipping or rotating; if a slice had been turned upside down between being sheared from the block face and being imaged under the microscope, or if it had diffused beyond 45 degrees from its angle on the block face. Several sets of downsamples of varying factors were generated, the lowest of which were used for debugging and testing, working up in detail, size and computational expense as the techniques were perfected.
    
    downsampling and smoothing: motivate downsampling with fact that HiRes moving image is needlessly detailed compared to LoRes (include in-plane dims and show that factor of 8 is optimal). Motivate Gaussian smoothing with Nernquist limit, noisiness and with drawing info from other pixels. For the reasons delineated in  Section~\ref{ssub:multiresolution_registration},  block-face downsamples were generated of all powers of 2 between 2 and 8, and histology downsamples between 2 and 64.
  ...
  \label{sub:initialisation}
    The optimisation algorithms used in registration do not assure convergence to the global minimum and are thus sensitive to initialisation and to the presence of local minima in the cost function. A reasonable initialisation is necessary for robust and accurate registration. The block face images are already coherent from their acquisition, and the slice images have been closely cropped such that the heart tissue lies at the centre. A simple manual alignment of the stack volumes relative to each other therefore provided adequate initialisation.
