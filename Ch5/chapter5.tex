...
\subsection{Aims and Hypotheses} % (fold)
\label{sec:aims}
  ...
  \begin{figure}[htbp]
    ...
    \caption{A meandering phase singularity from \cite{Valderrabano:2003p386} along a rabbit epicardial artery. Seven sequential phase maps are shown with the trajectory of the  singularity in the bottom right. The path is tightly restricted to the underlying artery.}
...
\label{sec:defining_the_laminae_of_equal_helical_angle}
  It is observed that the myocardial fibres that constitute ventricular tissue are organised such that their angle from the horizontal plane varies transmurally, from around $60^\circ$ to $-60^\circ$ \cite{StreeterJr:1969p597}. This angle from the horizontal is known as the helical angle. In order to incorporate transmurally varying mean fibre direction into the model \cite{Potse:2006p446}, every point in the tissue model must be assigned to a myocardial lamina, a curved surface containing contiguous muscle fibres, all of equal helical angle.
  ...
  The simplest model of myocardial laminae in the presence of a blood vessel, from here on referred to as the `simple model', ignores the effect of the vessel completely. The laminae are placed exactly as they would have been with no vessel, but terminate on the vessel boundaries and are undefined within the vessel. In the case of a cuboid wall segment, where $y$ is defined as the transmural direction, this is equivalent to saying that each lamina is the set of points that share the same $y$ value: a flat $x$-$z$ plane.
	...
	\begin{figure}[htbp]
		\centering
		\subfigure[][]{\includegraphics[height=55ex]{Ch5/Figs/histology}}
		\qquad
    \subfigure[][]{\includegraphics[height=55ex]{Ch5/Figs/cylinder}}
    \caption{\textbf{(a)} A 10µm-thick histological image of rabbit mid-myocardial wall long-axis slice, provided by \cite{Burton:2006p100}. \textbf{(b)} The equipotentials (vertical lines) and field lines (horizontal with arrow heads) of a conducting circle in an infinite uniform field derived from the analytical solution to Eq. \ref{eqn:potential}.}
	  \label{fig:vessel}
	\end{figure}
  ...
  \begin{figure}[htbp]
    \centering
    \subfigure[][A vessel at zero potential in red, and an equipotential just below zero in blue.]{\label{fig:coplanar1}\includegraphics[width=0.7\textwidth]{Ch5/Figs/coplanar1}}
    \subfigure[][The same vessel with an equipotential just above zero.]{\label{fig:coplanar2}\includegraphics[width=0.7\textwidth]{Ch5/Figs/coplanar2}}
    \subfigure[][An equipotential further above zero.]{\label{fig:coplanar3}\includegraphics[width=0.7\textwidth]{Ch5/Figs/coplanar3}}
  ...
  \begin{figure}[htbp]
    \centering
    \subfigure[][Low $\Phi$.]{\label{fig:perpendicular1}\includegraphics[width=0.7\textwidth]{Ch5/Figs/perpendicular1}}
    \subfigure[][Zero $\Phi$.]{\label{fig:perpendicular2}\includegraphics[width=0.7\textwidth]{Ch5/Figs/perpendicular2}}
    \subfigure[][High $\Phi$.]{\label{fig:perpendicular3}\includegraphics[width=0.7\textwidth]{Ch5/Figs/perpendicular3}}
  ...
  \begin{figure}[htbp]
    \centering
    \subfigure[][Low $\Phi$.]{\label{fig:oblique_no_vessel1}\includegraphics[width=0.469\textwidth]{Ch5/Figs/oblique_no_vessel1}} \qquad
    \subfigure[][Low $\Phi$ with rendered vessel.]{\label{fig:oblique_vessel1}\includegraphics[width=0.469\textwidth]{Ch5/Figs/oblique_vessel1}}
    \subfigure[][Zero $\Phi$.]{\label{fig:oblique_no_vessel2}\includegraphics[width=0.469\textwidth]{Ch5/Figs/oblique_no_vessel2}} \qquad
    \subfigure[][Zero $\Phi$ with rendered vessel.]{\label{fig:oblique_vessel2}\includegraphics[width=0.469\textwidth]{Ch5/Figs/oblique_vessel2}}
    \subfigure[][High $\Phi$.]{\label{fig:oblique_no_vessel3}\includegraphics[width=0.469\textwidth]{Ch5/Figs/oblique_no_vessel3}} \qquad
    \subfigure[][High $\Phi$ with rendered vessel.]{\label{fig:oblique_vessel3}\includegraphics[width=0.469\textwidth]{Ch5/Figs/oblique_vessel3}}
...
\label{sec:defining_the_fibre_direction_within_the_laminae}
  ...
  \subsubsection{Projecting a 2-D Solution} % (fold)
  \label{sub:projecting_a_2_d_solution}
    ...
    \begin{figure}[htbp]
      \centering
      \subfigure[][A view along the $x$-axis, showing the vessel intersecting the surface.]{\label{fig:projected1}\includegraphics[width=1\textwidth]{Ch5/Figs/projected1}}
      \subfigure[][The same view, with no vessel rendering and translucent surface rendering. The quivers are more visible and it is clear that none have a $y$-component.]{\label{fig:projected2}\includegraphics[width=1\textwidth]{Ch5/Figs/projected2}}
      \subfigure[][A view along the $y$-axis. The vectors in the $x$-$z$ plane have a direction trend up and to the left, at $90^\circ$ to $\alpha$. However, at the edge of the ellipse, where the vessel intersects the lamina, all vectors are perpendicular to the elliptical projection of the circle onto the $x$-$z$ plane.]{\label{fig:projected3}\includegraphics[width=1\textwidth]{Ch5/Figs/projected3}}
      \caption{A 3-D quiver plot of the compressed 2-D solution, plotted on the same isosurface as in Figures~\ref{fig:oblique_vessel} and \ref{fig:oblique_no_vessel}.}
      \label{fig:projected}
    \end{figure}

    The questions remains, in which orientation should solution~\ref{eqn:potential} be mapped? The following dependency of overall fibre direction on depth of myocardial tissue is observed experimentally \cite{Potse:2006p446}, \cite{StreeterJr:1969p597}. Let $e$ be a tissue depth parameter
    ...
    \begin{figure}[htbp]
      \centering
      \subfigure[][Depth parameter $e$ against $\alpha$.]{\label{fig:transmural_variation1}\includegraphics[width=0.469\textwidth]{Ch5/Figs/transmural_variation1}} \qquad
      \subfigure[][Depth parameter $e$ against unit direction vectors plotted in the x-z plane.]{\label{fig:transmural_variation2}\includegraphics[width=0.469\textwidth]{Ch5/Figs/transmural_variation2}}
  ...
  \label{sub:normalising_the_cross_product}
    ...
    \begin{figure}[htbp]
      \centering
      \vspace{-2.5em}
      \subfigure[][]{\label{fig:oblique_vessel1}\includegraphics[width=1\textwidth]{Ch5/Figs/fibres_vessel1}}
      \subfigure[][]{\label{fig:oblique_vessel2}\includegraphics[width=1\textwidth]{Ch5/Figs/fibres_vessel2}} 
      \subfigure[][]{\label{fig:oblique_vessel3}\includegraphics[width=1\textwidth]{Ch5/Figs/fibres_vessel3}}
    ...
    \begin{figure}[htbp]
      \centering
      \vspace{-2.5em}
      \subfigure[][]{\label{fig:oblique_no_vessel1}\includegraphics[width=1\textwidth]{Ch5/Figs/fibres_no_vessel1}}
      \subfigure[][]{\label{fig:oblique_no_vessel2}\includegraphics[width=1\textwidth]{Ch5/Figs/fibres_no_vessel2}} 
      \subfigure[][]{\label{fig:oblique_no_vessel3}\includegraphics[width=1\textwidth]{Ch5/Figs/fibres_no_vessel3}}
...
\label{sec:mesh_generation}
  ...
  \begin{figure}[htbp]
    \centering
    \subfigure[][A 10x10x3mm mesh generated from a 100$\mu$m resolution voxel image with {[0,0,1]} vessel direction and 1mm radius.]{\includegraphics[width=0.6\textwidth]{Ch5/Figs/tetgen}}
    \subfigure[][An enlarged view of the same mesh.]{\includegraphics[width=0.6\textwidth]{Ch5/Figs/tetgen_zoom}} 
  ...
  \begin{figure}[htbp]
    \centering
    \subfigure[][]{\includegraphics[width=0.6\textwidth]{Ch5/Figs/tarantula}}
    \subfigure[][]{\includegraphics[width=0.6\textwidth]{Ch5/Figs/tarantula_zoom}} 
  ...
  There were several formatting issues associated with the final workflow. Once Tarantula had generated a tetrahedral mesh from the voxel image file built in Matlab, A C++ script written by Anton Prassl was used to convert the various Tarantula file formats so that they could be interpreted by the electrophysiological simulation package CARP (Cardiac Arrhythmias Research Package) \cite{Vigmond:2003p657}. Tarantula generates both an intracellular (tissue) mesh and a surrounding extracellular box, so CARP was then used to extract out just the intracellular space from the total mesh, for the purposes of monodomain simulations.
...
\label{sec:electrophysiological_simulation}  
  Once each tetrahedron has been assigned a conductivity tensor, based on the modelled fibre direction at its centroid, simulations were run in CARP, using the UCLA rabbit ventricular action potential model \cite{Mahajan:2008p996}. Simulations were run in parallel on the Oxford OeRC cluster and took approximately a few hundred seconds to compute 300ms of activation on 32 cores.
...
\label{sec:results}
  ...
  \begin{figure}[htbp]
		\centering
		\subfigure[][Vertical MRI slice.]{\includegraphics[height=51ex]{Ch5/Figs/vertical_mri}}
		\qquad
    \subfigure[][Extracted vascular tree.]{\includegraphics[height=51ex]{Ch5/Figs/extracted_vessels}}
    \caption{\textbf{(a)} A 2-D image in the coronal plane from a 3-D Rabbit MRI dataset at resolution 26.4×26.4×24.4$\mu$m from \cite{Burton:2006p100}. Circular and elliptical cavities distributed within the myocardium can be observed.  These are the major vessels supplying the heart. \textbf{(b)} The vascular tree was extracted from the MRI dataset by Bishop et al \cite{Bishop:2009p444}. The main vessels are positioned either sub-epicardially or transmurally.}
	  \label{fig:vessels}
	\end{figure}
  ...
  \subsubsection{Planar Propagation} % (fold)
  ...
  \label{sub:induced_arrhythmia}
    ...
    \begin{figure}[htbp]
  		\centering
  	    \includegraphics[width=0.95\textwidth]{Ch5/Figs/shock_induced_558ms}
    ...
    \begin{figure}[htbp]
  		\centering
  	    \includegraphics[width=0.95\textwidth]{Ch5/Figs/shock_induced_1530ms}
              \caption{At 1530ms, the accurate arrythmia (bottom) has dissipated and tissue will restore to a polarised state, whereas the simple arrhythmia has fractionated into several wavefronts, showing complex 3-D dynamics.}
