...
\label{sec:methods}
  Intro: Summary sentence or comment sentence, then list content.
	...
	\label{sub:transformational_diffusion}
	  ...
	  How would this $\mathbf{\Delta T}_i^{n,n+1}$ be defined? In the case of 1-dimensional diffusion, we might reformulate the right hand side of Equation~\ref{eqn:diffusion_1d} into two separate terms thusly:
    ...
    where $\Delta f_i^{m,n}$ is the difference in $f$ at position $i$ between timestep $m$ and timestep $n$, and $\Delta f_{i,j}^m$ is the difference in $f$ between position $i$ and position $j$ at timestep $m$. From Equation~\ref{eqn:1d_diffusion_operator}, we can see that at each timestep, the concentration at each bin moves towards its left and right neighbour by a proportion $\alpha$ of its difference from each, respectively.
	  ...
	  We might, then, propose analogously that
		...
		\begin{equation}
		 	\mathbf{\Delta T}_i^{n,n+1} = \alpha \cdot \mathbf{\Delta T}_i^{n,n+1} \oplus \alpha \cdot \mathbf{\Delta T}_i^{n,n+1},
		\end{equation}
	 	
	 	where $\mathbf{\Delta T}_{i,j}^n$ is defined as the transformation that registers a histological slice $i$ to slice $j$ at iteration $n$, and $\cdot$ and $\oplus$ are placeholders for binary operators on a pair of transforms. The question now emerges: what precisely are the two binary operators? We will start by outlining their desirable properties, and then decide on a sensible implementation.
		
	  The operator $\cdot$ takes two operands: a scalar and a transform. We would like 
	 	
	  In the particular case of affine transformations, an ITK transform $\mathbf{T}$ acting on a point $\mathbf{p}$ consists of a matrix transformation $\mathbf{M}$, followed by a translation by an offset vector $\mathbf{o}$:
		
		
		DISCUSS SMALL LIMIT
		It is interesting to note how the two operators behave in the small limit...
	\begin{equation}
